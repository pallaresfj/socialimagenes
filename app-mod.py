...
@app.route('/UploadComentario/<id>', methods=['GET','POST'])
def upload_comentario(id):
    if not session:
        return redirect( url_for( 'login' ) )
    if request.method == 'POST':
        id=id
        ...
        db=get_db()
        sql = 'INSERT INTO tbComentarios (Id, IdImagen, IdUsuario, Comentario) VALUES (NULL,?,?,?);'
        datos = (id,session['id_usuario'],comentario)
        db.execute(sql,datos)
        db.commit()
    else: 
